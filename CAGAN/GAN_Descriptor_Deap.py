...
from cultural_evolution import gaMuPlusLambda
...
def eval_gan_fid(individual):
    ...
    if fid_score < best_val:
        best_val = fid_score
        torch.save(individual.GAN.Gen_network, "results/bestG.pkl")
        torch.save(individual.GAN.Disc_network, "results/bestD.pkl")

        SaveArchitectureToFile(individual,'bestGan')
...
def mut_gan(individual,belief_space):
    """
    ...
    if type_mutation == "add_layer":             # We add one layer
        ...
        x = aux_network.list_act_functions[layer_pos]
        ...
        aux_network.list_act_functions[layer_pos] = x
        ...
        my_gan_descriptor.lossfunction = fmeasure
        my_gan_descriptor.Gen_network.lossfunction = fmeasure
        my_gan_descriptor.Disc_network.lossfunction = fmeasure
    return individual,
...
def aplly_ca_gan(toolbox, pop_size=10, gen_number=50, cxpb=0.7, mutpb=0.3):
    """
...
def printNetwork(obj):
    ...
    return  network
def SaveArchitectureToFile(ind,filename):
    # Save the network architecture to the text file
        fname =path+'results/ind_'+str(filename)+'.txt'
        file1 = open(fname, "w")
...
if __name__ == "__main__":
    ...
    toolb = init_ga()
    # Runs the GA
    res, logbook, hof = aplly_ca_gan(toolb, pop_size=npop, gen_number=ngen, cxpb=CXp, mutpb=Mxp)

    # Save the FID and time score to the csv file
    for ind in res:
        fname = path+'results/ind_'+str(ind.GAN.indi_no)+'.csv'
    ...
    # Save the network architecture to the text file
    for ind in res:
        SaveArchitectureToFile(ind,ind.GAN.indi_no)
